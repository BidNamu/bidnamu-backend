package com.bidnamu.bidnamubackend.auction.repository.repositoryImpl;
...
public class AuctionRepositoryImpl extends QuerydslRepositorySupport implements
    AuctionRepositoryCustom {
    ...
    public Page<Auction> findBySearchAuction(final SearchAuctionRequestDto requestDto,
        ...
        JPAQuery<Auction> query = queryFactory
            ...
            .where(eqTitle(requestDto.title()),
                eqCategoryName(requestDto.name()),
        ...
        if (requestDto.sortMethod() != null) {
            query = eqSortMethod(query, requestDto.sortMethod());
        }

        final List<Auction> content = query
    ...
    private BooleanExpression eqAuctionStatus(final String auctionStatus) {
        return StringUtils.hasText(auctionStatus) ? auction.status.eq(
            AuctionStatus.valueOf(auctionStatus)) : null;
    }
